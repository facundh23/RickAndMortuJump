...
const Modal = ({ image, open, children }: Props) => {
  return (
    <div className={`fixed h-[100%] inset-0 bg-black/80 z-10 flex justify-center items-center transition-colors ${open ? "visible blur-20" : "invisible"}`}>
      ...
      <div onClick={e => e.stopPropagation()} className={`bg-gradient-to-b from-green-900 to-blue-500 rounded-xl h-100  shadow p-4 m-4 transition-all ${open ? "scale-100" : "scale-125 opacity-0"}`} >
